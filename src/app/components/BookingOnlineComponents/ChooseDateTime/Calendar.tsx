...
const Calendar: React.FC<CalendarProps> = ({ selectedDate, onDateChange, onClose }) => {
  ...
  const isPastDate = (date: Date) => {
    ...
    return date.getTime() < today.getTime(); // Compare the timestamps
  };
  ...
  return (
    <div className="absolute bg-white shadow-lg p-4 rounded-md w-64 z-50">
      ...
      <div className="grid grid-cols-7 text-center gap-1">
        ...
        {getDates().map((date) => {
          const isDisabled = isPastDate(date); // Check if the date is past
          return (
            <button
              ...
              onClick={() => !isDisabled && handleDateClick(date)} // Prevent click if disabled
              disabled={isDisabled}
            >
              {date.getDate()}
            </button>
